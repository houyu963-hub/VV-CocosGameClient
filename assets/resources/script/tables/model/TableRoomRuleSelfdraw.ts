...
export default class TableRoomRuleSelfdraw {
	/**
	 * ID
	 */
	public ID: number
	...
	public static parse = (bytes: ArrayBuffer): Map<number, TableRoomRuleSelfdraw> => { 
		let reader = new TableReader(new DataView(bytes))
		let map: Map<number, TableRoomRuleSelfdraw> = new Map()
		while (reader.offset < bytes.byteLength) {
			const data = new TableRoomRuleSelfdraw();
