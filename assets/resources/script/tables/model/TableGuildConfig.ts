...
export default class TableGuildConfig {
	/**
	 * ID
	 */
	public ID: number
	...
	public static parse = (bytes: ArrayBuffer): Map<number, TableGuildConfig> => { 
		let reader = new TableReader(new DataView(bytes))
		let map: Map<number, TableGuildConfig> = new Map()
		while (reader.offset < bytes.byteLength) {
			const data = new TableGuildConfig();
